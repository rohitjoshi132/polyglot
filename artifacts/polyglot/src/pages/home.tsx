    ...
    <div className="flex flex-col h-full gap-0">
      ...
      <div className="flex flex-col md:flex-row gap-5 flex-1 min-h-[540px]">
        ...
        <div className="flex-1 flex flex-col glow-border rounded-2xl glass-panel overflow-hidden border border-border/60 focus-within:border-primary/40 focus-within:shadow-[0_0_30px_-8px_hsl(152_72%_47%/0.2)] transition-all duration-300">
          {/* Editor Top Bar */}
          <div className="flex items-center gap-3 px-4 py-2.5 bg-black/30 border-b border-border/50">
            {/* macOS-style dots */}
          ...
          {/* Textarea */}
          <div className="relative flex-1 bg-[#080810]">
            <textarea
              ref={textareaRef}
              value={code}
              onChange={(e) => setCode(e.target.value)}
              placeholder={"// Paste or type your source code here...\n// Press Ctrl+Enter to run, Ctrl+Shift+D to detect language"}
              className="absolute inset-0 w-full h-full p-5 bg-transparent border-none outline-none font-mono text-[13.5px] leading-relaxed text-zinc-300 placeholder:text-zinc-800 resize-none"
              spellCheck={false}
            />
          </div>

          {/* Editor Footer Actions */}
          <div className="px-4 py-3 bg-black/30 border-t border-border/50 flex flex-wrap items-center justify-between gap-3">
            {/* Language override dropdown */}
            <div ref={dropdownRef} className="relative">
              <button
                onClick={() => setLangDropdownOpen((v) => !v)}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-black/40 border border-white/5 text-xs font-mono text-muted-foreground hover:border-primary/30 hover:text-primary transition-all"
              >
              ...
              <AnimatePresence>
                {langDropdownOpen && (
                  <motion.div
                    ...
                    className="absolute bottom-full mb-1.5 left-0 z-50 w-48 glass-panel rounded-xl border border-border/60 shadow-2xl overflow-hidden"
                  >
          ...
          <AnimatePresence mode="wait">
            {!showDetection && !showCompilation ? (
              ...
              >
                ...
                <div className="p-5 bg-gradient-to-br from-secondary/60 to-transparent border-b border-border/50">
                ...
                <Tabs defaultValue={showCompilation ? "output" : "signals"} className="flex-1 flex flex-col">
                  ...
                  {showCompilation && (
                    <TabsContent value="output" className="flex-1 p-0 m-0 flex flex-col overflow-hidden">
                      ...
                      <div className="flex-1 overflow-y-auto p-4 bg-[#060610] font-mono text-[13px]">
